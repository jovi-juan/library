...
function mh_theme_css($media='all'){
	...
	return '<link href="'.WEB_PUBLIC_THEME.'/'.$themeName.'/css/screen.css" media="'.$media.'" rel="stylesheet">';
}
...
function mh_item_metadata_additional($item='item',$html=null){
	$fields = all_element_texts($item,array('return_type'=>'array')); 
	//un-comment to see the items in the array, useful if you are wanting to see property names, etc.
	//return print_r($fields); 
	...
	if($fields['Physical Object Item Type Metadata']){
		$itm = array_filter($fields['Physical Object Item Type Metadata'],function($key){
			$omit=array('Location','URL','Latitude','Longitude');
			return !(in_array($key, $omit));
		},ARRAY_FILTER_USE_KEY); 
		
		// Output
		foreach($itm as $itmname=>$values){
	...
	if($fields['Text Item Type Metadata']){
		$itm = array_filter($fields['Text Item Type Metadata'],function($key){
			$omit=array('Location','URL','Latitude','Longitude');
			return !(in_array($key, $omit));
		},ARRAY_FILTER_USE_KEY); 
